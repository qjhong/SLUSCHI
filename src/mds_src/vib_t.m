...
[n_elms,n_atoms,mass,elms,n_atoms_total,stepsize,T,POS,LATT,VEL,step_unit] = read_files('s_800');
fid = fopen('phonon_dos_v9.out','r');
...
S_id(i_elm)=(5/2 + log(V/lambda^3) )*k_B*6.023e23;
    nunu = data(idx:idx+nn-1);
    idx = idx + nn;
    SS = data(idx:idx+nn-1);
    idx = idx + nn*3;
