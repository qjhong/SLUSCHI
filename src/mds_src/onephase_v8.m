...
for i_elm = 1:n_elms
  ...
  if SS(1) < max_dos * 0.1
    ...
    for i=1:nn-1
        ...
        %F(i+1) = k * T * log( 2 * sinh( h*nu_curr / (2*k*T) ) ) * (S(i)+S(i+1))/2*nu_max/(n-1);
        nu_curr = ( nunu(i)+nunu(i+1) ) / 2;
        F_1_c(i+1) = T * k_B * log( 2 * sinh( h*nu_curr / (2*k_B*T) ) )  * (SS(i)+SS(i+1))/2*nu_max/(nn-1);
        F_2_c(i+1) = T * max(-S_id(i_elm)/3/6.023e23+k_B, k_B * log( 2 * sinh( h*nu_curr / (2*k_B*T) ) ) ) * (SS(i)+SS(i+1))/2*nu_max/(nn-1);
        E_1_c(i+1) = h*nu_curr/2 *  (exp(h*nu_curr/k_B/T)+1) / (exp(h*nu_curr/k_B/T)-1) * (SS(i)+SS(i+1))/2*nu_max/(nn-1);
        area_c = area_c + (SS(i)+SS(i+1))/2*nu_max/(nn-1);
        % 3 translational degrees of freedom are missing in VASP
        % assuming: 1. they are the softest modes in DOS
        %           2. they are equally distributed among elements
        % 3 DOF distributed in n_atoms(i_elm) / sum(n_atoms)
    ...
    print('-f1','-r600','-dpng',strcat(filename,'_S_',elms{i_elm}));
    E = 3*k_B*T/eV
    EE_1_c = (sum(E_1_c))/area_c*3/eV;
    %E_1 = (sum(E_1)+E_1_c)/(area+area_c)*3/eV
    %FF_1 = (sum(F_1)-S_id/6.023e23*T)/area*3/eV;
    FF_1_c = (sum(F_1_c))/area_c*3/eV;
    S_p_1_c(i_elm) = -(FF_1_c-EE_1_c)/T*96485;
    %FF_1 = (sum(F_1)+F_1_c)/(area+area_c)*3/eV;
    %S_p_1(i_elm) = -(FF_1-E)/T*96485;
    %S_p_1(i_elm) = -(FF_1-E_1)/T*96485;
    %FF_2 = (sum(F_2)-S_id/6.023e23*T)/area*3/eV;
    FF_2_c = (sum(F_2_c))/(area_c)*3/eV;
    %S_p_2(i_elm) = -(FF_2-E_1)/T*96485
    %FF_2 = (sum(F_2)+F_2_c)/(area+area_c)*3/eV;
    %S_p_2(i_elm) = -(FF_2-E)/T*96485;
    S_p_2_c(i_elm) = -(FF_2_c-EE_1_c)/T*96485;
    area_c
    %area_1 = area;
    %S_p_1_1 = S_p_1(i_elm);
    %S_p_2_1 = S_p_2(i_elm);
    S_p_1(i_elm) = - ( sum(F_1-E_1) + (sum(F_1_c-E_1_c)-sum(F_1-E_1))/(area_c - area) * 3 * n_atoms(i_elm) / sum(n_atoms) ) / ( area + 3 * n_atoms(i_elm) / sum(n_atoms) ) * 3 / eV / T * 96485;
    S_p_2(i_elm) = - ( sum(F_2-E_1) + (sum(F_2_c-E_1_c)-sum(F_2-E_1))/(area_c - area) * 3 * n_atoms(i_elm) / sum(n_atoms) ) / ( area + 3 * n_atoms(i_elm) / sum(n_atoms) ) * 3 / eV / T * 96485;
    % number of DOF to add: 3 * n_atoms(i_elm) / sum(n_atoms)
    % number of DOF 0: area
    % number of DOF 1: area_c
    % int 0: sum(F_x) with area
    % int 1: sum(F_x_c) with area_c
    %area_id / area
    n_int = area;
    end
    fprintf(fid, '%6.2e\n', nunu);
