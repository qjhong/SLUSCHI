function [S_xlogx,S_plogp,S_plogp_merge1,S_plogp_merge2,R_cut0,n_NN,avg_iter,R_x,R_anal,count_anal,elms] = pdf_v6(filename,idx_in1,idx_in2,flag_solid,avg_iter)
...
set(gcf, 'Color', 'w');
print('-f1','-r600','-dpng',strcat(filename,'_R_anal'));
...
for i = 2:n_R
    ...
    if R_anal(i)<R_anal(i-1)-1e-10 && flag_increase == 1 && R_anal(i-1) > 0.1 * R_max % make sure this is a large peak
        flag_decrease = 1;
    end
...
set(gcf,'PaperPositionMode','auto')
set(gcf, 'Color', 'w');
print('-f1','-r600','-dpng',strcat(filename,'_count_anal_',elms{idx_in1},'_',elms{idx_in2}));
