...
#        atomind = [1] * int(natms[0]) + [2] * int(natms[1])      
        ...
        #print(pos)
        #time.sleep(5)
        print(" Based on the last %d MD steps, \tthe spacegroup of the structure is %s (tol = 0.05) \t %s (tol = 0.1) \t %s (tol = 0.2) \t %s (tol = 0.5) \t %s (tol = 1.0) \t %s (tol = 2.0) \t %s (tol = 3.0)." 
              % ( position_his.shape[0]//c, spglib.get_spacegroup(strct,symprec=5e-2), spglib.get_spacegroup(strct,symprec=1e-1), spglib.get_spacegroup(strct,symprec=3e-1), spglib.get_spacegroup(strct,symprec=5e-1), spglib.get_spacegroup(strct,symprec=10e-1), spglib.get_spacegroup(strct,symprec=20e-1), spglib.get_spacegroup(strct,symprec=30e-1) ) )
    print("")
